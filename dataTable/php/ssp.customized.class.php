...
class SSP {
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        $c_extraWhere = $extraWhere;

		// IF Extra where set then set and prepare query
        if($extraWhere) {

            $c_extraWhere = "WHERE $extraWhere";
        ...
        if(strlen($where) > 0 ) {
            ...
            $extraWhere = "WHERE 1 $extraWhere";
        }

        $w = isset($c_extraWhere) ? " $c_extraWhere " : "1";

        $q = "SELECT COUNT(`{$primaryKey}`) FROM `$table` $w ";

        error_log(__FILE__.".".__LINE__." ::::: ".$q);
