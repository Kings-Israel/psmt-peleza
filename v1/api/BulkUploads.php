...
class BulkUploads
{
    ...
    public $config;
    public $logger;
    public static $ONE_MB = 1048576; // bytes
    private $mailer;
    ...
    public function postData($index)
    {

        if (isset($_POST['request_type'])) {

            error_log("SUBMITTING REQUEST $index ");
            ...
            switch ($_POST['request_type']) {
                ...
                case "module":
                    ...
                    } else if ($data_type == "none" || $data_type == "hidden") {
    ...
    public function submit($index)
    {
        ...
        $consent_form = isset($general['consent_form']) ? $general['consent_form'] : "";
        $cost = $general['cost'];
        ...
        foreach ($modules as $module) {
            ...
            if ($data_type == 'file') {
                ...
                $sql = "INSERT INTO pel_psmt_files (psmtfile_name,psmtfile_type,psmtfile_filetoken,request_id,client_id, data_type)
                                                    VALUES ('$value','$document_name','$BLOCKCHAIN','$BLOCKCHAIN','$MM_client_id','file')";
            ...
            } else if ($data_type !== 'hidden' || $data_type !== 'none') {
                $value = $module['value'];

                $sql = "INSERT INTO pel_psmt_files (psmtfile_name,psmtfile_type,psmtfile_filetoken,request_id,client_id, data_type)
                                                    VALUES ('$value','$document_name','$BLOCKCHAIN','$BLOCKCHAIN','$MM_client_id','text')";
    ...
    public function uploadFile($name)
    {
        ...
        if (isset($_FILES[$name])) {
            ...
            if (empty($errors) == true) {
                ...
                $filename = $name . "_" . microtime(true);
                ...
                $mp = "/var/www/html/temp/" . $file_name;

                move_uploaded_file($file_tmp, $mp);

                $space = new SpacesConnect($this->config->spaces->key, $this->config->spaces->secret, $bucket, $region);
                $space->UploadFile($mp, "public", "$spaceName/$filename", mime_content_type($mp));
...
if (isset($_POST['request_ref_number'])) {
    ...
    $report = new BulkUploads();
    $report->postData($index);
